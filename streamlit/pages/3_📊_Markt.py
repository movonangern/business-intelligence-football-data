...
def calculate_weighted_percentile(player_id, columns, weights, position, top_n=100):
    ...
    sorted_values = sorted(all_weighted_values, reverse=True)
    top_values = sorted_values[:top_n]

    if weighted_player_value in top_values:
        percentile = (top_values.index(weighted_player_value) + 1) / len(top_values) * 100
    else:
        percentile = (sum(1 for v in sorted_values if v < weighted_player_value) + 1) / len(sorted_values) * 100
...
def calculate_metrics(player_id, position):
    ...
    for metric, data in metrics.items():
        ...
        percentile = calculate_weighted_percentile(player_id, columns, weights, position)
